...
import {
  ...
  ChevronLeft, ChevronRight, ChevronDown, Users, Wallet,
...
export default function ExpensesPage() {
  const {
    ...
    expensesExportOpen: showExport, setExpensesExportOpen: setShowExport,
  ...
  const [monthFilt,      setMonthFilt]     = useState(() => {
    ...
    if (f?.year) return 'all' // drill-down nga Dashboard-i sipas vitit — shfaq krejt muajt e atij viti
  ...
  const [recurringOpen,  setRecurringOpen] = useState(() => {
    try { return localStorage.getItem('arestv_expenses_recurring_open') === 'true' } catch { return false }
  })
  useEffect(() => {
    try { localStorage.setItem('arestv_expenses_recurring_open', String(recurringOpen)) } catch {}
  }, [recurringOpen])
      ...
      {recurringItems.length > 0 && (
        <div className="mb-6">
          ...
          >
            ...
            <ChevronDown size={15} className={`text-gray-400 group-hover:text-gray-600 transition-transform ${recurringOpen ? 'rotate-180' : ''}`} />
          </button>
          {recurringOpen && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {recurringItems.map(e => (
              <div
                ...
                className="text-left bg-white rounded-2xl border border-gray-200/90 shadow-sm px-3.5 py-3 flex items-center justify-between gap-3 hover:shadow-md transition-all cursor-pointer"
              >
